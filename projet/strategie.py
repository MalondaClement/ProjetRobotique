...
class StratCercle(object):
   def __init__(self, robot, rayon, temps, direction, cercle) :
       ...
       self.cercle=cercle
       self.distance= (((2*pi*(rayon+robot.WHEEL_BASE_WIDTH) )*(cercle/100))*(self.robot.WHEEL_CIRCUMFERENCE/360))
   ...
   def step(self):
        ...
        if self.direction=="gauche" :
            ...
            self.robot.set_motor_dps(1, ((2*pi*(self.rayon+self.robot.WHEEL_BASE_WIDTH)/(self.cercle/100))/self.temps)*(360/self.robot.WHEEL_BASE_CIRCUMFERENCE))
            self.robot.set_motor_dps(2, ((2*pi*self.rayon/(self.cercle/100))/self.temps)*(360/self.robot.WHEEL_BASE_CIRCUMFERENCE))
